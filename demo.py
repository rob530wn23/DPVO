...
if __name__ == '__main__':
    ...
    if args.plot:
        traj_ref = np.loadtxt('datasets/P006/pose_left.txt', delimiter=" ")[:, [1, 2, 0, 4, 5, 3, 6]]
        Path("trajectory_plots").mkdir(exist_ok=True)
        plot_trajectory(pred_traj, gt_traj=traj_ref, title=f"DPVO Trajectory Prediction for {name}", filename=f"trajectory_plots/{name}.pdf")
